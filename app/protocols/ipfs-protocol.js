export default async function makeIPFSFetch (opts = {}) {
  const errLog = opts.err
    ...
    const app = await (async () => { if (finalOpts.helia) { return finalOpts.helia; } else {const {createHelia} = await import('helia');const {FsDatastore} = await import('datastore-fs');const {FsBlockstore} = await import('blockstore-fs');const {identify} = await import('@libp2p/identify');const {kadDHT} = await import('@libp2p/kad-dht');const {gossipsub} = await import('@chainsafe/libp2p-gossipsub');const {bootstrap} = await import('@libp2p/bootstrap');const { tcp } = await import('@libp2p/tcp');return await createHelia({blockstore: new FsBlockstore(repo), datastore: new FsDatastore(repo), libp2p: {addresses: {listen: ['/ip4/0.0.0.0/tcp/0']}, transports: [tcp()], peerDiscovery: [bootstrap({list: ['/ip4/104.131.131.82/tcp/4001/ipfs/QmaCpDMGvV2BGHeYERUEnRQAwe3N8SzbUtfsmvsqQLuvuJ','/dnsaddr/bootstrap.libp2p.io/ipfs/QmNnooDu7bfjPFoTZYxMNLWUQJyrVwtbZg5gBMjTezGAJN','/dnsaddr/bootstrap.libp2p.io/ipfs/QmQCU2EcMqAqQPR2i9bChDtGNJchTbq5TbXJJ16u19uLTa']})], services: {dht: kadDHT(), pubsub: gossipsub(), identify: identify()}}});} })()
